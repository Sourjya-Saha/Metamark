...
// ========== NEW: EXTRACT URLS FROM TEXT FILE ==========
...
startBulkBtn.addEventListener('click', async () => {
  if (!bulkFileInput.files || !bulkFileInput.files[0]) {
    addLog('Please select a .txt file first', 'error');
    return;
  }
  ...
  reader.onload = async (e) => {
    ...
    const bulkUrls = extractUrlsFromText(text);

    if (bulkUrls.length === 0) {
      addLog('No valid URLs found in file', 'error');
      return;
    }

    addLog(`Starting bulk processing for ${bulkUrls.length} URLs`, 'info');
    ...
    chrome.runtime.sendMessage({
      ...
      if (chrome.runtime.lastError) {
        addLog('Bulk start error: ' + chrome.runtime.lastError.message, 'error');
        return;
      }
      if (resp && resp.started) {
        addLog('Bulk processing started', 'success');
      } else {
        addLog('Could not start bulk processing', 'error');
      }
    });
  };
