...
// ========== NEW: CHECK IF SEARCH PAGE ==========
function isSearchPage() {
  const url = window.location.href;
  return (
    (url.includes('amazon') && (url.includes('/s?') || url.includes('/s/'))) ||
    (url.includes('flipkart') && url.includes('/search?'))
  );
}
...
// ========== NEW: SHOW SEARCH PAGE UI ==========
function showSearchPageUI() {
  updateOverlay(`
    <div class="metamark-status">🔍 Search Page Detected</div>
    <div class="metamark-details">Click below to analyze top 3 products from this search</div>
    <button class="metamark-button" id="metamark-start-search-bulk">Start Bulk Processing</button>
  `);
  
  const startBtn = document.getElementById('metamark-start-search-bulk');
  if (startBtn) {
    startBtn.addEventListener('click', handleSearchBulk);
  }
}

// ========== NEW: HANDLE SEARCH PAGE BULK ==========
async function handleSearchBulk() {
  chrome.storage.sync.get(['isLoggedIn'], async (storage) => {
    ...
    try {
      showLoading('📡 Extracting product links from search page...');
      
      const searchUrl = window.location.href;
      
      // Call your existing backend endpoint
      const response = await apiRequest('/extract-links', {
        method: 'POST',
        body: { 
          url: searchUrl,
          num_links: 3
      ...
      if (!response.success || !response.data || response.data.length === 0) {
        showError('No product URLs found from search page');
        return;
      }
      
      // Extract just the URLs from the response data
      const urls = response.data.map(item => item.url);
      
      updateOverlay(`
        <div class="metamark-status">✅ Found ${urls.length} products</div>
        <div class="metamark-details">Starting bulk compliance check...</div>
      `);
      
      // Send to background for sequential processing
      chrome.runtime.sendMessage({
        action: 'startBulkProcessing',
        urls: urls
      }, (resp) => {
        if (chrome.runtime.lastError) {
          showError('Bulk start error: ' + chrome.runtime.lastError.message);
          return;
        }
        if (resp && resp.started) {
          updateOverlay(`
            <div class="metamark-status">🚀 Bulk processing started</div>
            <div class="metamark-details">Processing ${urls.length} products...</div>
          `);
        } else {
          showError('Could not start bulk processing');
        }
      });
      ...
      if (error.message.includes('401') || error.message.includes('Session expired')) {
        ...
        showError(error.message || 'Failed to extract search products');
      }
    }
  });
}
...
// ========== NOTIFY BULK DONE ==========
function notifyBulkDone(errorFlag = false) {
  chrome.runtime.sendMessage({ action: 'bulkItemDone', error: errorFlag }, () => {});
}
...
function handlePageChange() {
  ...
  if (isProductPage()) {
    ...
    createOverlay();
    setTimeout(() => showSearchPageUI(), 1000);
  }
}
...
if (isExtensionEnabled && isProductPage()) {
  createOverlay();
  setTimeout(() => checkCompliance(), 1000);
} else if (isExtensionEnabled && isSearchPage()) {
  createOverlay();
  setTimeout(() => showSearchPageUI(), 1000);
}
